...
class SelectionMonitor {
  ...
  private func checkCurrentSelection() {
    ...
    if let webArea = findWebArea(element: appElement) {
      ...
      if AXUIElementCopyAttributeValue(webArea, kAXSelectedTextAttribute as CFString, &selectedText)
        == .success,
  ...
  private func checkClipboard() {
    guard
      let clipboardString = NSPasteboard.general.string(forType: .string)?.trimmingCharacters(
    ...
    processSelectedText(clipboardString)
  }
  ...
  private func startMonitoring() {
    ...
    if bundleID == "com.apple.Safari" {
      // Safari特殊处理：定时轮询
      safariPollingTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) {
        [weak self] _ in
  ...
  private func addNotificationRecursively(
    element: AXUIElement, observer: AXObserver, selfPtr: UnsafeMutableRawPointer
  ) {
    // 尝试添加通知
    ...
    if AXUIElementCopyAttributeValue(element, kAXFocusedUIElementAttribute as CFString, &focused)
      == .success,
      let focusedElement = focused, focusedElement !== element
    {
      addNotificationRecursively(
        element: focusedElement as! AXUIElement, observer: observer, selfPtr: selfPtr)
    }
  }
  ...
  private func handleSelectionChange(element: AXUIElement) {
    ...
    if !isMouseDown {
      var selectedText: AnyObject?
      if AXUIElementCopyAttributeValue(element, kAXSelectedTextAttribute as CFString, &selectedText)
        == .success,
        let text = selectedText as? String, !text.isEmpty
      {
        processSelectedText(text)
