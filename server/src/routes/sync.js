...
router.get('/favorites', async (req, res) => {
  try {
    const user = await req.user.populate('favorites.term');
...
router.put('/favorites', async (req, res) => {
  try {
    ...
    if (!Array.isArray(favorites)) {
      return res
        .status(400)
        .json({ error: 'Favorites must be an array', code: 'INVALID_FAVORITES' });
    }
...
router.put('/vocabulary', async (req, res) => {
  try {
    ...
    if (!Array.isArray(vocabulary)) {
      return res
        .status(400)
        .json({ error: 'Vocabulary must be an array', code: 'INVALID_VOCABULARY' });
    }
